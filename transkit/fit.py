...
def minimize_refine(
    ...
    params_func: Callable[..., lm.Parameters],
    methods: Iterable[str],
    init_params: Optional[lm.Parameters] = None,
    residuals_args: Optional[tuple] = None,
    params_args: Optional[tuple] = None,
    verbose: bool = False,
) -> RefinedFit:
    """
    Minimize a function with lmfit using a succession of methods.

    This is useful if, for example, you want to find a first approximate
    solution using a randomized global optimizer like basinhopping, and
    then want to fine tune the result of that using something like
    least_squares.

    Returns a RefinedFit namedtuple of: (
        params:       Lmfit Parameters object containing the optimized
                      solution after all minimization steps.
        result:       Lmfit MinimizerResult object from the final
                      minimization step.
        all_results:  List of MinimizerResult objects from each
                      minimization step.
        init_params:  The initial parameters used for the first
                      minimization step.
    )

    Parameters:
    ===========
    residuals_func  The objective function returning the residuals that
                    will be minimized by lmfit. This should be a
                    Callable that takes an lmfit Parameters object as
                    the first argument, followed by any number of extra
                    positional arguments that can be specified in
                    `residuals_args`.
    params_func     A Callable that generates an appropriate lmfit
                    Parameters object for residuals_func. It must have a
                    keyword argument `seed=None` which takes an existing
                    Parameters object from which the new Parameters
                    should be initialized. If seed is unspecified or
                    None this function should return default values for
                    the parameters. It's signature can also have
                    positional arguments that can be specified in
                    `params_args`.
    methods         An Iterable of strings, each specifying a
                    minimization method that is supported by lmfit. Each
                    method will be run in succession, with the result of
                    the previous method being used as the initial values
                    for the next method.
    init_params     Optional lmfit Parameters object to use as initial
                    parameters for the first minimization method. If
                    None, the initial parameters will be generated by
                    calling `params_func` with no seed.
    residuals_args  Optional tuple of extra positional arguments that
                    will be passed to residuals_func.
    params_args     Optional tuple of positional arguments that will be
                    passed to params_func.
    verbose         Optional boolean to turn on step-by-step output of
                    each successsive minimization.
    """

    # TODO: pick the best fit
    # TODO: consider running randomized methods more than once and
    #       picking the best?

    if init_params is None:
        if params_args is None:
            init_params = params_func()
        else:
            init_params = params_func(*params_args)

    all_results = []
    for method in methods:
        seed = all_results[-1].params if len(all_results) > 0 else init_params
        if params_args is None:
            params = params_func(seed=seed)
        else:
            params = params_func(*params_args, seed=seed)

        if verbose:
            params.pretty_print()

        minimizer = lm.Minimizer(
            residuals_func, params, fcn_args=residuals_args
        )
